 ...
 * @description Stub {@link FfmpegMcuAdapter} that spawns an ffmpeg child
 *   process per mix.  Implements the {@link McuAdapter} contract by
 *   wiring mediasoup `PlainTransport`s to ffmpeg's RTP I/O — the exact
 *   pattern documented in the mediasoup recording sample.
 *
 *   This file deliberately keeps the spawn logic *minimal* and gated:
 *   if `ffmpeg-static` is not installed the constructor throws so apps
 *   get a clear "install ffmpeg-static" error rather than a silent
 *   no-op.  The wire-up between ffmpeg's RTP output and the SFU's
 *   ingest (re-publishing the mixed track as a new producer) is left
 *   for the integrator to implement against their specific SFU adapter
 *   — every adapter exposes the necessary primitives (`createTransport`
 *   with `plainRtp`, `produce`).
 *
 *   For a fully managed equivalent, prefer LiveKit egress (already
 *   wired through {@link LiveKitSfuAdapter#startRoomCompositeEgress}).
 */
