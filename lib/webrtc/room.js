 ...
 *
 *   A `Room` holds a set of `Peer`s plus a fluent chain of policy gates
 *   that decide whether a given peer may join (`require()`), publish
 *   (`canPublish()`), or subscribe (`canSubscribe()`).  Membership is
 *   process-local; pair the hub with `useCluster()` to share state across
 *   multiple Node processes behind a load balancer.
 *
 *   You normally never call `new Room(name)` directly —
 *   `hub.room(name)` constructs one lazily and returns the same instance
 *   on subsequent calls.  Removed-from-hub rooms (after `close()`) are
 *   garbage collected once you drop the reference.
 */
