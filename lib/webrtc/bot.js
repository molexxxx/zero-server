 ...
 *   `opts.wrtc`.
 *
 *   The bot is **fully bidirectional**: it accepts inbound tracks via
 *   `onTrack` and exposes the underlying `RTCPeerConnection`s through
 *   `getPeerConnection(remotePeerId)` so you can `addTrack()` /
 *   `createDataChannel()` for outbound media.
 *
 *   Use cases:
 *
 *   - **Recording bot** — pipe inbound tracks into `wrtc`'s
 *     `RTCAudioSink` / `RTCVideoSink` and write to disk.
 *   - **Transcription bot** — forward audio frames to Whisper / Deepgram
 *     and `room.broadcast('caption', ...)` the result.
 *   - **AI participant** — generate TTS audio with an `RTCAudioSource`
 *     and `pc.addTrack()` it back to the room.
 *   - **Integration tests** — prove the signaling \u2194 media path end-to-end
 *     against a real SFU.
 */
'use strict';
