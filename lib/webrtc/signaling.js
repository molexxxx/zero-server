 ...
 *   surface is acceptable.  In production you bind a `SignalingHub` to a
 *   Zero Server `app.ws()` upgrade handler; in tests a tiny `EventEmitter`
 *   shim works just as well.
 *
 *   **Wire protocol (JSON frames):**
 *
 *   | direction | `type`       | required fields                              |
 *   | --------- | ------------ | -------------------------------------------- |
 *   | C → S    | `join`       | `room`, optional `token`                     |
 *   | C → S    | `leave`      | —                                            |
 *   | C → S    | `offer`      | `sdp` (RFC 8829)                             |
 *   | C → S    | `answer`     | `sdp`, `to` (target peerId)                  |
 *   | C → S    | `ice`        | `candidate`, `to`                            |
 *   | C → S    | `mute`       | `kind: 'audio' | 'video'`                    |
 *   | C → S    | `unmute`     | `kind`                                       |
 *   | C → S    | `bye`        | optional `reason`                            |
 *   | C ↔ S    | `e2ee-key`   | opaque `key`, `to` (relayed without inspect) |
 *   | S → C    | `hello`      | `peerId` (sent on attach)                    |
 *   | S → C    | `joined`     | `roster: [{id, user}]` (sent on successful join) |
 *   | S → C    | `peer-joined`/`peer-left` | `{id, user}` broadcast to room     |
 *   | S → C    | `error`      | `code`, `message`                            |
 *
 *   All inbound frames are size-checked (`maxSdpSize`), rate-limited
 *   (`peerMessageRate`), and counted against `maxProtocolErrors`.  Per-IP
 *   attach floods are throttled with `ipAttachRate`.
 *
