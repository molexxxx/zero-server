 ...
 *
 *   A `Peer` wraps a transport object (typically the `WebSocketConnection`
 *   handed to your `app.ws()` handler, but any duck-typed
 *   `{ send, on, close }` works — see the unit tests for a minimal
 *   in-memory transport) and exposes the JSEP message types as ordinary
 *   events.  The hub is responsible for routing.
 *
 *   You very rarely construct `Peer` directly; `hub.attach(transport, info)`
 *   returns one.  The interesting surface is the `send` / `sendError` /
 *   `close` triad and the read-only fields (`id`, `user`, `ip`, `state`,
 *   `room`, `errors`, `connectedAt`, `closed`) which are stable across
 *   versions and safe to log.
 *
