 ...
 *
 *   `bindObservability(hub, { metrics, tracer })` subscribes to the hub's
 *   lifecycle events and exports the six standard `zs_webrtc_*` Prometheus
 *   series plus per-operation OpenTelemetry-compatible spans.  Both are
 *   opt-in — pass only the registry you want.
 *
 *   The metrics adapter is duck-typed: any object with
 *   `counter()` / `gauge()` / `histogram()` factory methods that return
 *   `inc()` / `dec()` / `observe()` style instruments will work.  Zero
 *   Server's built-in `app.metrics()` (from `@zero-server/observe`)
 *   satisfies the contract out of the box, as do `prom-client`'s
 *   `Counter`/`Gauge`/`Histogram` if you wrap them in a thin adapter.
 */
