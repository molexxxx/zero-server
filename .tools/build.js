	...
	item.commandGroups = [
		{
			...
			commands: [
				{ cmd: 'npx zs make:migration <name>',  desc: 'Auto-generate a migration by comparing your current Model schemas against the last snapshot. The CLI writes the <code>up()</code> and <code>down()</code> code for you - no manual SQL. Pass <code>--empty</code> to get a blank template instead.', args: '--dir=<path> --models=<path> --empty' },
				{ cmd: 'npx zs make:model <name>',     desc: 'Create a new Model class with a table name, primary key, and <code>timestamps</code> enabled.', args: '--dir=<path>' },
				{ cmd: 'npx zs make:seeder <name>',     desc: 'Create a new Seeder class for inserting sample or default data.', args: '--dir=<path>' },
			],
		},
		{
			label: 'Seeding',
			...
			commands: [
				{ cmd: 'npx zs seed', desc: 'Run every seeder file in the seeders directory.' },
			],
		},
		{
			...
			commands: [
				{ cmd: 'npx zs migrate',          desc: 'Run all pending migrations in order. Tracks each run in a numbered batch for easy rollback.' },
				{ cmd: 'npx zs migrate:rollback',  desc: 'Undo the last batch of migrations. Calls each migration\'s <code>down()</code> in reverse order.' },
				{ cmd: 'npx zs migrate:status',    desc: 'Show which migrations have been applied and which are still pending.' },
				{ cmd: 'npx zs migrate:reset',     desc: 'Rollback every migration, then re-run them all from the start.' },
				{ cmd: 'npx zs migrate:fresh',     desc: 'Drop <strong>all</strong> tables and re-run every migration from scratch. Useful during early development.' },
				{ cmd: 'npx zs migrate:remove',    desc: 'Delete the most recent migration file - but only if it hasn\'t been applied yet. Also reverts the schema snapshot.' },
			],
		},
		{
			label: 'Info',
			commands: [
				{ cmd: 'npx zs help',    desc: 'Print the full help message with all commands and options.' },
				{ cmd: 'npx zs version', desc: 'Print the installed zero-server version.' },
			],
		},
	];
	...
	item.workflows = [
		{
			...
			steps: [
				{ label: '1. Create a config file', code: '// zero.config.js\nmodule.exports = {\n    adapter:       \'sqlite\',\n    connection:    { filename: \'./app.db\' },\n    migrationsDir: \'./migrations\',\n    seedersDir:    \'./seeders\',\n    modelsDir:     \'./models\',\n};' },
				{
					...
					tabs: [
						{ tab: 'Scaffold new', code: '$ npx zs make:model User\n$ npx zs make:model Post' },
		...
		},
		{
			...
			steps: [
				{ label: '1. Scaffold a seeder', code: '$ npx zs make:seeder Users' },
				...
				{ label: '3. Run your seeders', code: '$ npx zs seed\n\n  Running seeders...\n    ✓ UsersSeeder\n\n  1 seeder(s) completed.' },
			],
		},
		{
			tab: 'Undo & Redo',
